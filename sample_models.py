...
def final_model(input_dim=161, filters=200, kernel_size=11, conv_stride=2,
    ...
    for i in range(recur_layers):
        # rnn_layer = LSTM(units, return_sequences=True, implementation=2, dropout=0.2, recurrent_dropout=0.2)(input_rnn)
        rnn_layer = Bidirectional(GRU(units, return_sequences=True, activation='relu',
                       implementation=2, dropout=0.2))(input_rnn)
        input_rnn = BatchNormalization()(rnn_layer)
